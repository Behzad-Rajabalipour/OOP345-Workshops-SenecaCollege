...
int main() {
  ...
  // CheeseParty testing
  {
    ...
    printdivider(70, "Slice Cheeses[0-1] - buffalo gouda, bulgarian feta");
    cheeses[0]->slice(100); // cheeses[0] pointer hast
    cheeses[1]->slice(100); 
    printdivider(70);
    ...
    cp = cp2;                   // age cp ro be cp3 vasl konim memory leak mishe. chon faghat pointer array delete shode, va pointer item ha delete nashodan. CheeseParty.cpp => #ref11
    std::cout << cp;
    printdivider(70);
  ...
  // cleanup
  for (auto i = 0u; i < cnt; ++i)           // CheeseParty.cpp => #ref11
    delete cheeses[i];
  delete[] cheeses;
